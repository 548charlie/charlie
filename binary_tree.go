...
func (n *Node )  Insert(value, data string)  error {
    if n == nil {
        return errors.New("Can not inert a vlaue into a nil tree" ) 
    } 
...
func (n *Node) replaceNode(parent, replacement *Node) error {
    if n == nil {
        return errors.New("repalceNode() not allowed on a nil node " ) 
    } 
    if  n == parent.Left {
...
func main() {
    ...
    for i := 0; i < len(values) ; i++ {
        ...
        if err != nil {
            log.Fatal("error inerting value", values[i], ":", err   ) 
        }
    }
