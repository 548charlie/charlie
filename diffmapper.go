...
usage with two parameters to compare files
diffmapper <file1 pattern|filename> <files2 pattern|filename2>

eg: diffmapper c:\dinakar\c_test\hello.c c:\dinakar\c_test\hello1.c

or to compare all files with some pattern

diffmapper c:\dinakar\c_test\*hello*.c c:\dinakar\c_test\*hel*.c

usage with four parameters:
...
func compareMaps (lines1, lines2 map[string]int, env1, env2 string ) {
    ...
    */
    ...
    if len(keys) > 0 {  
        ...
        fmt.Println("No differences found between two") 
    } 
